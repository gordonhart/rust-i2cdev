...
//! I2C communications protocol trait definitions.
...
/// **M**anagement **Bus** (SMBus) protocol, a more widely compatible subset of
/// the complete I2C protocol. See the [Linux Kernel SMBus protocol
...
pub trait I2CDevice {
    ...
    ///
    /// This is the opposite operation as `smbus_read_byte`.
    fn smbus_write_byte(&mut self, value: u8) -> Result<(), Self::Error> {
        self.write(&[value])
    }

    /// Read a single byte from a device from a designated register specified
    ...
    /// Write a single byte to a specific register on a device specified through
    /// the Comm byte.
